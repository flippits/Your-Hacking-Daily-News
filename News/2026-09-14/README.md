# Your Hacking Daily News — 2026-09-14

_A clean, daily hacking magazine with the best stories, advisories, and research — in plain language._

![Hacker illustration](https://upload.wikimedia.org/wikipedia/commons/thumb/0/06/Password_hacking_illustration.jpg/1280px-Password_hacking_illustration.jpg)

---

## At a Glance

- Top Stories (fast read, clear takeaways)
- Vulns & Patches (what to fix first)
- Research & Exploits (deep dives and PoCs)

---

## Editor's Note

Fresh security updates and threat intel highlights, distilled for a quick read — no digging required.

![Security cover](https://upload.wikimedia.org/wikipedia/commons/thumb/0/06/Password_hacking_illustration.jpg/1280px-Password_hacking_illustration.jpg)

## Analyst’s Pick

### CVE-2026-85921 Windows Secure Kernel Mode Elevation of Privilege Vulnerability
_Source: Microsoft Security Update Guide (RSS) · 2026-09-14_

**Why it’s worth your time:** Double free in Windows Secure Kernel Mode allows an authorized attacker to elevate privileges locally.

_Read more:_ https://msrc.microsoft.com/update-guide/vulnerability/CVE-2026-85921

## Fast Facts

- Total items scanned: 64
- Top Stories: 1 · Vulns: 4 · Research: 0
- This Week’s Trend keywords: catalog, cisa, distillation, chromium, claude

## This Week’s Trend

Across sources, the most repeated topics are **catalog, cisa, distillation, chromium, claude**.

## Top Stories

### The Self-Expanding Stolen Inference Supply Chain: An AI Agent Harvesting and Re-Serving LLM Access, (Fri, Sep 11th)
_Source: SANS ISC Diary (Title Only) · 2026-09-11_

**Mini‑article:** I identified an attacker using a semi-autonomous coding agent to run an offensive operation: finding poorly secured LLM resale gateways, acquiring API access through ordinary web flaws and account farming, validating the resulting inference capacity, and aggregating it behind a single gateway of their own.

_Read more:_ https://isc.sans.edu/diary/rss/33332

## Vulns & Patches

### CVE-2026-85921 Windows Secure Kernel Mode Elevation of Privilege Vulnerability
_Source: Microsoft Security Update Guide (RSS) · 2026-09-14_

**Mini‑article:** Double free in Windows Secure Kernel Mode allows an authorized attacker to elevate privileges locally.

_Read more:_ https://msrc.microsoft.com/update-guide/vulnerability/CVE-2026-85921

### CVE-2026-62721 Windows User-Mode Power Service (UMPS) Elevation of Privilege Vulnerability
_Source: Microsoft Security Update Guide (RSS) · 2026-09-14_

**Mini‑article:** The CVE was updated with links to security updates for Windows 11, version 26H1, 25H2, and 24H2 to address a missed fix. Microsoft recommends installing these updates as soon as possible.

_Read more:_ https://msrc.microsoft.com/update-guide/vulnerability/CVE-2026-62721

### Chromium CVE-2026-87491:  Out of bounds write in V8
_Source: Microsoft Security Update Guide (RSS) · 2026-09-12_

**Mini‑article:** This CVE was assigned by Chrome. Microsoft Edge (Chromium-based) ingests Chromium, which addresses this vulnerability.

_Read more:_ https://msrc.microsoft.com/update-guide/vulnerability/CVE-2026-87491

### Chromium CVE-2026-76023:  Improper resource control in Linux Toolkit Theming
_Source: Microsoft Security Update Guide (RSS) · 2026-09-12_

**Mini‑article:** This CVE was assigned by Chrome. Microsoft Edge (Chromium-based) ingests Chromium, which addresses this vulnerability.

_Read more:_ https://msrc.microsoft.com/update-guide/vulnerability/CVE-2026-76023

## Research & Exploits

No items today.

---

_More tomorrow. Stay patched and stay sharp._

_Image credits:_ Password hacking illustration by Santeri Viinamäki (CC BY-SA 4.0); Password hacking illustration by Santeri Viinamäki (CC BY-SA 4.0)

